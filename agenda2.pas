...
procedure abrirCriar(var fAgenda:fContatos;var matAgenda:matContatos);
...
Begin
	...
	writeln('qual o nome da agenda para criar/abrir ?');
	readln(nome);
	assign(fAgenda,nome);
	{$I-}
	reset(fAgenda);
...
Begin
	...
	begin
		...
		writeln('insira o nome do novo contato');

		readln(mAgenda[posi].rName);

		lerLongint( mAgenda[posi].rNum,0,2147483647,'insira o número do contato.Ele deve ser um inteiro entre 0 e 2147483647');
...
BEGIN
	...
	abrirCriar(agendaPrincipal,matrizPrincipal);
	...
	repeat	  
			...
			lerInteiro(choice,1,5,'escolha uma opção inteira entre 1 e 5');
